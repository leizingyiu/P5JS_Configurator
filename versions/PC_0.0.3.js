...
// Last modified: "2023/11/06 17:29:49"
...
defaultVal, minVal, maxVal, precision need number`);
    ...
    checkbox(name = 'p5js_ctrler_checkbox', defaultVal = false, labelText = ['yes', 'no'], fxn = () => { }) {
        ...
        const labelRetext = (label, newContext) => {
            if (Boolean(label.querySelectorAll('input').length && label.querySelectorAll('span').length)) {
                label.querySelector('span').textContent = newContext;
            } else if (!Boolean(label.querySelectorAll('*').length)) {
                label.textContent = newContext;
            } else {
                console.error('cant find text node of checkbox');
            }
        }

        let label = this.ctrlers[name].elt.querySelector('label');

        labelRetext(label, labelText[this.ctrlers[name].checked() ? 0 : 1]);
